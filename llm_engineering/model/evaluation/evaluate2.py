...
DATASET_HUGGINGFACE_WORKSPACE = 'tanghan6'
MODEL_HUGGINGFACE_WORKSPACE = 'Qwen'
...
def generate_answers(model_id: str, dataset_name: str):
    ...
    # for split_name, split in my_dataset.items():
    #     split.push_to_hub("tanghan6/llmtwin", split=split_name)
    ...
    sampling_params = SamplingParams(temperature=0.8, top_p=0.95, min_p=0.05, max_tokens=1024 )
    ...
    # dataset.push_to_hub(f"tanghan6/{model_id.split('/')[-1]}-results")
    # del llm
    gc.collect()
...
def evaluate_answer(instruction: str, answer: str, client: AzureOpenAI) -> dict:
...
def evaluate_batch(batch, start_index):
    client = AzureOpenAI(
        ...
        api_version="2024-05-01-preview",  # Replace with your desired API version
...
model_ids = [
    check_if_huggingface_model_exists(
        "openai-community/gpt2", default_value="openai-community/gpt2"        
    ),
    check_if_huggingface_model_exists(
        f"{MODEL_HUGGINGFACE_WORKSPACE}/Qwen2.5-0.5B-Instruct", default_value="Qwen/Qwen2.5-0.5B-Instruct"    
    ),    
    # check_if_huggingface_model_exists(
...
if __name__ == "__main__":
    ...
    for model_id in model_ids:
        dataset = load_dataset(f"{DATASET_HUGGINGFACE_WORKSPACE}/{model_id.split('/')[-1]}-results", split="all")
        # dataset.push_to_hub(f"tanghan6/{model_id.split('/')[-1]}-results", split="all")
